...
### ⚙️ Анатомія та життєвий цикл запиту в `AnalyticsResolver` (Як це працює під капотом)

У той час як у REST API контролер (`@Controller`) прив'язаний до фіксованого URL-шляху і повертає жорстко заданий JSON, **GraphQL-резолвер працює як навігатор по графу даних**:

1. **Ідентифікація типу сутності (`@Resolver(() => CategoryReport)`)**:
   - Повідомляє NestJS, що даний клас відповідає не лише за глобальні операції, а й за резолвінг специфічних властивостей сутності `CategoryReport`.
2. **Маршрутизація до точки входу (`@Query()`)**:
   - Apollo Server аналізує вхідний AST-документ і знаходить відповідний обробник `categoryReports`.
   - Декоратор `@Args('filter')` автоматично парсить JSON-аргументи клієнта, інстанціює клас `DateRangeInput` і проганяє його крізь валідаційні пайплайни (`class-validator`).
3. **Делегування бізнес-логіки**:
   - Через механізм Dependency Injection викликається `AnalyticsService.getCategoryRevenueReport()`, який виконує оптимізований конвеєр агрегацій у MongoDB.
4. **Трансформація даних (Data Mapping)**:
   - Отримані з MongoDB пласкі агреговані записи трансформуються у відповідність до моделі `CategoryReport` (прив'язка `id` та вкладеного об'єкта `metrics`).
5. **Гранулярне обчислення полів (`@ResolveField()`)**:
   - Якщо клієнт **не запросив** `formattedSummary`, відповідний метод **взагалі не викликається** (Lazy Evaluation — нульові зайві обчислення).
   - Якщо ж поле запитано, GraphQL ітерується по кожному елементу масиву, передає його в аргумент `@Parent() report: CategoryReport`, обчислює відформатований рядок і підшиває його у відповідь.
6. **Field Masking (Маскування полів)**:
   - GraphQL відкидає всі незапитані поля, повертаючи клієнту виключно ті дані, які були оголошені у вибірці (Zero Over-fetching).
